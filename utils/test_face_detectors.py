		...
		for i in range(0, detections.shape[2]):
			...
			if confidence > self.confidence:
				...
				if (w > 0 and h > 0):
					...
					#self.c=self.c+1
					#cv2.waitKey(0)
					cv2.rectangle(image,(startX,startY),(startX+w,startY+h),(255,0,0),2)
...
class cascade:
	...
	def __init__(self,save_path):
		self.dt = cv2.CascadeClassifier('../models/haar-cascade-files/haarcascade_frontalface_default.xml')
		self.c = 0
		self.save_folder = save_path
...
def from_camera():
	...
	while ( cap.isOpened() ):
		...
		if ret:
			...
			frameDetector = dt1.run(frame)
			frameCascade = dt2.run(frame)
			
			cv2.imshow("Detector", frameDetector)
			cv2.imshow("Cascade", frameCascade)
